(() => {
        ...
        <div class="bg-gradient-to-r from-blue-600 to-blue-700 text-white p-4 flex items-center justify-between">
          <div class="flex items-center space-x-3">
            <div class="w-8 h-8 bg-white bg-opacity-20 rounded-full flex items-center justify-center p-2">
              <svg xmlns="http://www.w3.org/2000/svg" class="lucide lucide-message-square-text" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/><path d="M13 8H7"/><path d="M17 12H7"/></svg>
            </div>
            <div>
          ...
          <button id="closeChat" class="text-white hover:text-blue-200 transition-colors">
            <svg xmlns="http://www.w3.org/2000/svg" class="lucide lucide-x" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
          </button>
        </div>
        ...
        <div class="p-4 border-t border-gray-200 bg-white">
          <div class="flex space-x-2">
            ...
            <button id="sendMessage" class="bg-blue-600 hover:bg-blue-700 text-white rounded-full p-2 transition-colors">
              <svg xmlns="http://www.w3.org/2000/svg" class="lucide lucide-send-horizontal" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3.714 3.048a.498.498 0 0 0-.683.627l2.843 7.627a2 2 0 0 1 0 1.396l-2.842 7.627a.498.498 0 0 0 .682.627l18-8.5a.5.5 0 0 0 0-.904z"/><path d="M6 12h16"/></svg>
            </button>
          </div>
        </div>
      </div>
      <button id="chatButton" class="w-14 h-14 bg-blue-600 hover:bg-blue-700 text-white rounded-full shadow-lg transition-all flex items-center justify-center relative">
  ...
  function handleMessageByType(data) {
    const { type, message, options = [], file = null } = data;

    if (message) addMessage(message, false);

    if (type === 1 && Array.isArray(options)) {
      const wrapper = document.createElement('div');
      wrapper.className = 'flex mb-3 justify-start';

      const container = document.createElement('div');
      container.className = 'bg-white text-gray-800 border rounded-tl-none max-w-xs px-4 py-2 rounded-lg text-sm shadow flex flex-wrap gap-2';

      options.forEach(opt => {
        const btn = document.createElement('button');
        btn.className = 'px-3 py-1 text-sm rounded-full border border-blue-600 text-blue-600 hover:bg-blue-600 hover:text-white transition';
        btn.textContent = opt.option;
        btn.onclick = () => {
          addMessage(opt.option, true);
          socket.send(JSON.stringify({ sender: 'user', message: opt.option }));
          wrapper.remove();
        };
    ...
    if (type === 2 && file) {
      ...
      const bubble = document.createElement('div');
      bubble.className = 'bg-white text-gray-800 border rounded-tl-none max-w-xs px-4 py-2 rounded-lg text-sm shadow flex flex-col gap-2';
      ...
      if (isImage) {
        ...
        icon.className = 'w-6 h-6 bg-gray-300 text-center rounded flex items-center justify-center text-xs font-bold';
        icon.textContent = isPDF ? 'PDF' : ext.toUpperCase();
